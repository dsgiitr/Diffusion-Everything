...
class DDPM():
        ...
        for epoch in range(numEpochs):
            ...
            for i, (img, y) in tqdm(enumerate(dataloader), total = len(dataloader)):
                ...
                with torch.autocast(device_type = self.device, dtype = torch.float16):
                    predictedNoise = self.UNet(encodedImages.half(), ts, y).sample
            ...
            # !! TESTING !!
            images = self.generate_ddpm(25, torch.randint(0, 10, (25, ), device = self.device))[-2]
    ...
    def generate(self, numImages, labels): 
