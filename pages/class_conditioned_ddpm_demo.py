...
if choice == "DDPM - Class Conditioned":
    ...
    if st.button("Generate"):
        ...
        if st.session_state["steps"] < 1000 :
            ddpm.fast_generate(1, torch.tensor([names.index(selected_name)], device = ddpm.device), steps = st.session_state["steps"], lerp = st.session_state["lerp"], streamlit_callback = partial_callback)
        else :
            ddpm.generate(1, torch.tensor([names.index(selected_name)], device = ddpm.device), streamlit_callback = partial_callback)
