...
import numpy as np
import torch
from classifier_guidance import DDPM
import numpy as np
...
menu = ["DDPM - Classifier Guidance", "Option 2", "Option 3"]
...
def load_models():
    ddpm_cosine = DDPM(
        ...
        betaStart = 1e-4, 
        betaEnd = 2e-2,
        UNetConfig = json.loads(open("classifier_guidance/config.json").read()),
        scheduler = "cosine",
        clf_checkpoint = "../checkpoints/clf.model",
        unet_checkpoint = "../checkpoints/unet.model", 
        device = "cuda:0", 
    )
    ddpm_linear = DDPM(
        timesteps = 1000, 
...
if choice == "DDPM - Classifier Guidance":
    st.header("DDPM - Classifier Guidance")
    ...
    st.latex(r"x_{t-1} = \frac{1}{\sqrt{\alpha_t}}(x_t - \frac{1 - \alpha_t}{\sqrt{1 - \bar{\alpha_t}}} \epsilon_{\theta}(x_t, t)) + s\sigma_t\nabla_{x_{t}}\log p_\phi(y|x_t) + \sigma_t z")
    ...
    st.session_state["guidance-scale"] = st.number_input("Guidance Scale", min_value = 0, step = 1)
    ...
    if st.button("Generate"):
        ...
        if st.session_state["steps"] < 1000 :
            ddpm.fast_generate(1, torch.tensor([names.index(selected_name)], device = ddpm.device), guidanceScale = st.session_state["guidance-scale"], steps = st.session_state["steps"], lerp = st.session_state["lerp"], streamlit_callback = partial_callback)
        else :
            ddpm.generate(1, torch.tensor([names.index(selected_name)], device = ddpm.device), guidanceScale = st.session_state["guidance-scale"], streamlit_callback = partial_callback)
